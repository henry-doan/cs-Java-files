...
public class ComparatorPractice {
	...
	public static void main(String[] args) {
		...
		String uid = "u0674937"; /* CHANGE THIS TO YOUR UID*/
		 ...
		 */
		List<Student> studentsOrderedByName = database.getStudentsOrderedBy(new NameComparator()); // or any other Comparator syntax.
		printStudentList(studentsOrderedByName); // comment me out to avoid printing out the entire list.
		...
		tester.sortByUid_ReverseGPA__age(new Comparator<Student>() {
			...
			public int compare(Student lhs, Student rhs) {
				...
				if (result == 0) {
					result = Double.compare(lhs.gpa, rhs.gpa);
					if(result == 0) {
						result = lhs.age - rhs.age;
					}
				}
				return result;
			}
		...
		tester.sortBySmallestNumberOfVowels_Dogs_GPA(new Comparator<Student>() {
			...
			public int compare(Student lhs, Student rhs) {
				...
				for(int index = 0; index < firstAndLast.length; index++) {
					String word = firstAndLast[index];
				...
				for(int index = 0; index < rtfirstAndLast.length; index++) {
					String word = rtfirstAndLast[index];
					String[] letters = word.split("");
