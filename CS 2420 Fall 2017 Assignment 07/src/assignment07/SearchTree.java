 ...
 * </p>
 * 
 * @author Peter Jensen and Henry Doan
 * @version November 5, 2017 November 18, 2017
 */
public class SearchTree<E>
{
    ...
    {
    	if (targetReference == null && treeHeight(root) != 0) {
    		throw new NullPointerException("The targetReference cannot be null");
    	}
    	...
    	Node current = root;
    	if (targetReference == tranverseDown(current, targetReference)) {
    		return tranverseDown(current, targetReference);
    	}
    ...
    /**
     * Transverse down the tree and checks if the compared ref matches the node. If it does, then return the node
     ...
     *        node
     *        	current node
     * @return the node if the node matching the comparedRef else return the current node
     * 
     */
    
    @SuppressWarnings("unchecked")
	public E tranverseDown(Node node, E comparedRef) {
    	Node current = node;
    	if( current.left != null ) {
    		if (current.left == comparedRef ) {
    			return (E) current.left;
    		}
    		current.left = (Node) tranverseDown(current.left, comparedRef);
    	}
        if( current.right != null ) {
        	if (current.right == comparedRef ) {
    			return (E) current.right;
    		}
        	current.right = (Node) tranverseDown(current.right, comparedRef);
        }
    ...
    {
    	n.height = Math.max(treeHeight(n.left), treeHeight(n.right)) + 1;
        int heightDiff = subtreeHeightDifference(n);
        if (heightDiff < -1) {
        	if (subtreeHeightDifference(n.right) > 0) {
        		rightRotate(n.right);
                leftRotate(n);
            } else {
                leftRotate(n);
        	...
        	if (subtreeHeightDifference(n.left) < 0) {
        		leftRotate(n.left);
                rightRotate(n);
            } else {
               rightRotate(n);
    ...
    public int subtreeHeightDifference(Node n){
		if(n == null){
			return 0;
		}
		return ((subtreeHeightDifference(n.left) - subtreeHeightDifference(n.right)));
	}
    
    /**
     * <p>
     * Returns height of the tree or
     * 0 if n is null.
     * </p>
     * 
     * @param n
    ...
    @SuppressWarnings("unchecked")
	public static void leftRotate(Node n) {
    	if (n.right == null) {
    		throw new NullPointerException("There is no right child");
    	}
    	Node rotate = n.right;
        n.right = rotate.left;
        rotate.left = n;
        n.height = Math.max(treeHeight(n.left), treeHeight(n.right)) + 1;
        rotate.height = Math.max(treeHeight(rotate.left), treeHeight(rotate.right)) + 1;
    }
    ...
    @SuppressWarnings("unchecked")
	public static void rightRotate(Node n) {
    	if (n.right == null) {
    		throw new NullPointerException("There is no right child");
    	}
        Node rotate = n.left;
